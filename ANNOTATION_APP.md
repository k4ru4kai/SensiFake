...
```bash
uv run streamlit run annotation_app.py -- --mode annotation --calibration
```
...
```bash
uv run streamlit run annotation_app.py -- --mode annotation --demo
```

Run the read-only presentation artifact after round 1 is complete:

```bash
uv run streamlit run annotation_app.py -- --mode presentation
```
